  ...
  return (
    <>
      ...
      <div className="hero mx-auto max-w-7xl grid md:grid-cols-2 gap-8 items-center px-4 sm:px-6 lg:px-8 py-10 pt-20 grid-cols-1">
        <div className="animate__animated animate__fadeInUp animate__delay-2s">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl leading-tight font-bold mb-6">
            Hai, saya Rizky Fadillah Hetharia
          </h1>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap items-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 bg-red-700 py-3 px-5 rounded-2xl hover:bg-red-800 w-full sm:w-auto justify-center touch-manipulation"
            >
              Download CV
            ...
            <a
              ...
              className="inline-flex items-center gap-2 bg-red-700 py-3 px-5 rounded-2xl hover:bg-red-800 w-full sm:w-auto justify-center touch-manipulation text-white"
            >
              Lihat Proyek
      ...
      <div className="tentang mt-32 py-10" id="tentang">
        <div className="xl:w-2/3 lg:w-3/4 w-full mx-auto p-6 md:p-8 bg-red-900 rounded-lg text-white" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="350">
          ...
          <p className="text-base leading-loose mb-6 md:mb-10">
            Hai — saya Rizky Fadillah Hetharia, siswa SMK yang sedang berproses menjadi
          ...
          {listTools.map((tool) => (
            <div key={tool.id} className="flex flex-col items-center p-2">
              <img
                src={tool.gambar}
                alt={tool.nama}
                className="w-12 sm:w-14 p-2 bg-red-800 rounded-md hover:bg-red-900 transition"
                loading="lazy"
              />
              ...
              <span className="mt-2 inline-block text-[11px] bg-red-700 px-2 py-0.5 rounded-full opacity-80 text-white">{tool.dad}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="proyek mt-32 py-10" id="proyek">
        ...
        <div className="proyek-box mt-14 grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="350">
          {listProyek.map((proyek) => (
            <div key={proyek.id} className="bg-red-900 rounded-lg overflow-hidden p-4 flex flex-col">
              ...
              <div className="flex flex-wrap mb-4 gap-2">
                {proyek.tools.map((tool, index) => (
                  <span key={index} className="inline-block bg-red-800 px-2 py-0.5 rounded-full text-[11px] opacity-80 mr-2 text-white">
                    {tool}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-auto text-center w-full">
                <a href="#" target="_blank" rel="noopener" aria-label={`Lihat website ${proyek.nama}`} className="inline-block w-full sm:w-auto bg-red-700 px-4 py-3 rounded-full text-white hover:bg-red-800">Lihat Hasil Proyek</a>
              </div>
            </div>
          ))}
        </div>
        ...
        >
          <div className="grid gap-4 flex flex-col gap-6" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="350">
            ...
            <div className="text-center grid gap-4 flex flex-col gap-2">
              <button type="submit" className="inline-block bg-red-700 px-4 py-2 rounded-full text-white hover:bg-red-800 border border-zinc-500 rounded-sm">Kirim Pesan</button>
            </div>
          </div>
        </form>
      </div>
